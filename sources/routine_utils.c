...
/*   Updated: 2024/05/17 12:48:20 by andre-da         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_sleep(suseconds_t time_to_wait, t_philo *philo)
{
	...
	while (true)
	{
		if (((suseconds_t)get_current_time() - start) 
			>= (suseconds_t)time_to_wait && death_check(philo))
			return ;
		usleep (100);
	}
}
...
bool	check_right_fork(t_philo *philo)
{
	...
	if (!philo->r_fork)
	{
		ft_sleep(philo->parms.die_time, philo);
...
void	eat(t_philo *philo)
{
	...
	ft_sleep(philo->parms.eat_time, philo);
